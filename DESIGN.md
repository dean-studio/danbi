...
colors-light:
  ...
  stone: "#d4d4d8"
  on-dark: "#0a0a0a"
  on-dark-mute: "rgba(10,10,11,0.72)"
  canvas: "#ffffff"
  surface: "#fafafa"
  ...
  button-fg: "#e4e4e7"
  hairline: "#e4e4e7"
  hairline-soft: "rgba(10,10,11,0.08)"
...
The system ships with two mirrored palettes: **dark (default)** and **light**. Every component, typography token, radius, spacing, and layout rule is identical across modes — only the `colors` palette swaps in/out. This keeps the Raycast chrome vocabulary (hairline borders, surface ladder, ss03 Inter, single CTA pill) intact in both modes and avoids mode-specific component variants.

- **Dark** — the canonical Raycast palette. CTA pill is `{colors.primary}` white on `{colors.on-primary}` black.
- **Light** — a mirrored palette under `colors-light`. The surface ladder runs white → off-white steps (`#ffffff` → `#fafafa` → `#f4f4f5` → `#eeeeef`), hairlines soften to `#e4e4e7`, and the CTA pill inverts to near-black (`#0a0a0a`) on white text so the "one solid pill per viewport" rule still reads.
- **Accents** (red/green/blue/yellow) shift one notch darker in light mode so contrast holds on white surfaces; they remain reserved for extension/feature illustrations, not chrome.
- **Hero stripe gradient** (`{colors.hero-stripe-start}` → `{colors.hero-stripe-end}`) is shared across both modes — the one saturated chromatic moment on chrome.
- **No drop shadows in either mode.** Elevation is still built from the surface-color ladder; the light ladder provides enough separation without introducing shadows.

**Token resolution.** When implementing, resolve `{colors.*}` against the active mode (`dark` → `colors`, `light` → `colors-light`). Components reference the same keys in both modes; there is no `{colors-light.primary}` syntax in component specs.
...
### Do
- Default to dark mode. Light mode is the mirrored secondary palette — render whichever mode is active in one continuous surface tone (no mid-page mode switches).
...
### Don't
- Don't fork component specs per mode. Components reference the same `{colors.*}` keys in both modes; only the palette table swaps.
...
- **Light mode is the mirrored palette only** — the captured Raycast surfaces are dark; the light palette was derived by inverting the surface ladder and adjusting accent contrast, not captured from a light-mode Raycast page.
